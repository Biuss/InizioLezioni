...
public class gestione {
	...
	/*public void coppiaVett (int vett[], int lungo) {
		int [] copiaVett = vett.clone();
		int temp = 0;
		int conta = 0;
		System.out.println("Ecco il vettore copia: ");
		for (int i = 0; i < vett.length; i ++) {
		System.out.print(copiaVett[i] + " ");
		}
		System.out.println(" ");
		for (int j = 0; j < vett.length; j++) {
			temp = copiaVett[j];
			if (temp == copiaVett[j + 1]) {
				conta++;
				System.out.println("Coppia di numeri: " + temp);
			}
				else if (conta != 2) {
					System.out.println("Ci sono 3 numeri in coppia o nessuna coppia");
			}
			temp = copiaVett[j + 1];
		}
	}*/
	...
	public void coppiaVett(int vett[], int lungo) {
		...
		if (flag == true) {
			System.out.println("La coppia di numeri comprende: " + temp);
		}
		else {
